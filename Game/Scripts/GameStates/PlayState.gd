...
enum PlayStateStatus {
    INITIAL,
    PLAYSTATE_STARTED, # start playstate
    PLAYING, # PlayState, player travels around and does stuff
    GOD, # Gods get a chance to chat
}
...
signal on_action_triggered(action_name: String)
...
func start():
    ...
    on_action_triggered.connect(self._on_action_triggered)

func _on_action_triggered(action_name: String):
    print("This is where we listen for actions like sleep to progress the playstate")
    print("Action name is " + action_name)
    if player.PLAYER_REST == action_name:
        _set_status(str(PlayStateStatus.GOD))
...
func _setup_world():
    ...
    var tavern: Tavern = Tavern.new()
    tavern.display_name = "TestTavern"
